...
- **Admin prehliadač logov** — záložka **Logy** v admin paneli (`/admin`) zobrazuje posledné riadky logu naživo (`GET /api/admin/logs`, len admin), s **filtrovaním podľa úrovne** (INFO/WARNING/ERROR/CRITICAL) a **textovým vyhľadávaním**, výberom počtu riadkov a voliteľnou auto-obnovou každých 10 s.
...
### 📈 Grafana — biznis dashboard

Biznis metriky (používatelia, PLUS/MRR/ARR, testy, DAU, úspešnosť, tržby) beží
v **Grafana Cloud (free)** napojenej priamo na Supabase Postgres cez read-only
rolu `grafana_ro` — bez zmien v aplikácii, **0 € navyše**.

- Rola vidí z tabuľky `users` len bezpečné stĺpce (**bez** `password` / `reset_token`)
  a má iba `SELECT`; tabuľky s RLS majú pre ňu select-only policies.
- Pripojenie ide cez Supabase **Session pooler** (TLS `require`).
- Hotový dashboard (15 panelov) sa importuje z JSON-u.

Kompletný návod, SQL skripty aj dashboard JSON: [`docs/grafana/`](docs/grafana/README.md).
...
- **Platby (Paddle):** kód hotový, sandbox E2E ✅, **účet je live** (doména schválená + KYC prešlo, 2026-07-10) ✅ — **zostáva live konfigurácia** (checkout settings, webhook, Cloud Run env, E2E s reálnou kartou — viď checklist v `TODO.md`)
